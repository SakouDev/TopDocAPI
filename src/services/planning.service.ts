...
export class PlanningService implements IService<PlanningDTO> {
    ...
    async findById(id: number): Promise<PlanningDTO | null> {
        ...
        if (today >= new Date(data.Holidays[0].startDate) && today <= new Date(data.Holidays[0].endDate)) {
            ...
            const minutesTotales = (data.Weekday[0].endHour.getTime() - data.Weekday[0].startHour.getTime()) / (1000 * 60)
            const nbCreneaux = Math.floor(minutesTotales / data.Weekday[0].duration)

            const CreneauxList = []

            for (let i = 0; i < nbCreneaux; i++) {
                const startHour = new Date(data.Weekday[0].startHour.getTime() + ((data.Weekday[0].duration * (1000 * 60)) * i)).toLocaleTimeString()
                const endHour = new Date(data.Weekday[0].startHour.getTime() + ((data.Weekday[0].duration * (1000 * 60)) * (i + 1))).toLocaleTimeString()
            ...
            const selectedDate = { jour: data.Weekday[0].today, creneaux: CreneauxList }
        }
